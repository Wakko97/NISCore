...
@app.get("/api/v1/modules/{run_id}/evidence", dependencies=[Depends(require_role("admin", "operator", "viewer"))])
def list_module_evidence(run_id: int) -> list[dict]:
    with get_session() as session:
        rows = session.exec(select(ModuleEvidence).where(ModuleEvidence.operation_run_id == run_id).order_by(ModuleEvidence.id.asc())).all()
    return [r.model_dump() for r in rows]
